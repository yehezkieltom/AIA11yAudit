...
const guideline1_4_3 = async (metadata: Metadata, imageB64Original: string, imagesB64Filtered: FilteredImage[], imageName: string) => {
    ...
    You will be provided with multiple image of a UI Design with different color filter that simulates color blindness and the original without color filter. Using the original image (filename suffixed with "_nofilter") mention the relative location of each element in the image. And then, for each element make a description of what you see in regard to the text styling like the coloring and its background color, text weight, case and font family. refer sans serif font as normal. After that, decide if it passed or failed the Success Criterion Contrast (Minimum) of Web Content Accessibility Guideline.
    Additionally check again the element(s) that passed the standard WCAG 1.4.3 in the image with color filter whether or not the element still has minimum contrast value required by WCAG 1.4.3, if no, then mark the element as "Failed"
    If the image consist of multiple component, only return "Passed" If all of the components are passing the this Success Criterion. Otherwise, return "Failed".
